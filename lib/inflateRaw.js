		...
		(function(){
				var utils = require("./utils/common")
					...
					, zlib_inflate = require("./zlib/inflate")
				...
				function Inflate(options) {
					var opt = this.options = utils.assign({
						chunkSize: 16384,
						windowBits: 0,
						to: ""
					}, options || {});
							...
							if(strm.next_out) {
								if(strm.avail_out === 0 || status === c.Z_STREAM_END || (strm.avail_in === 0 && _mode === c.Z_FINISH)) {
									if(this.options.to === "string") {
										...
										utf8str = strings.buf2string(strm.output, strm.next_out);
										strm.next_out = tail;
			...
			this.inflateRaw = function(input, options) {
				options = options || {};
				options.raw = true;
